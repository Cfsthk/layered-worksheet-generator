#!/usr/bin/env python3
"""Seven worksheets: loopback-only local application server.

Run `python3 serve.py`. Credentials are accepted per operation in a request
header, forwarded only to Alibaba's Hong Kong host, and never written to disk.
"""
import argparse
import copy
import json
...
def start_job(body, key):
    with LOCK:
        ...
        item = {"created": time.time(), "status": "running", "done": 0, "total": 1,
                "message": "準備連接香港模型…", "cancel": client.cancel}
