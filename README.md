...
網頁版在瀏覽器準備文件，直接以使用者的金鑰連接指定的官方 Qwen 端點。PDF（包括掃描件）逐頁轉成圖片；JPG、JPEG、PNG 直接作為圖片；DOC、DOCX 擷取文字及可讀內嵌圖片，再由 Qwen 整理題目與目標。有圖片時使用所選視覺模型，純文字文件使用所選文字模型。這符合 Qwen 的[多圖片輸入介面](https://www.alibabacloud.com/help/en/model-studio/vision)。

瀏覽器文件工具隨網站發佈，沒有第三方文件轉換服務。API Key 只用於當次模型請求，不包含在程式碼、GitHub 或瀏覽器儲存空間中。網頁版 HEIC 視乎瀏覽器支援，建議使用 JPG／PNG。

## 第一次使用

1. 按「連接 Qwen」，輸入所用 Workspace 的 API Key。在「模型與連線設定」的 API Host 貼上阿里雲提供的主機或完整 URL。「測試文字模型連線」會作一次小額請求，只驗證文字模型。
...
`npm test` 驗證網頁版的資料合約、目標保留、精確分數核對、指定主機、切換、取消及錯誤處理。另已在 Chrome 驗證六種真實檔案格式、PDF 逐頁圖片、輸入金鑰後接續讀取、生成及 Word 下載；模型回覆使用測試資料。

尚未使用使用者的真實 API Key 驗證帳戶模型權限、品質及速度。請在應用程式輸入指定 Workspace 的金鑰。
