...
function requireConnection() {
  ...
  if (!$("#api-key").value.trim()) {
    ...
    throw new Error(t("請先在 Qwen 設定輸入香港區域的 API Key，再繼續。", "Enter your Hong Kong API key in Qwen settings, then continue."));
  }
...
async function runLive(operation, payload) {
  ...
  try {
    ...
    $("#progress-dialog .field-note").textContent = t("香港連線 · 所需時間按文件和模型而異", "Hong Kong connection · Time depends on the document and model");
...
function afterTranslate() {
  ...
  copyLabel("#settings-dialog .region-note", "官方香港區域 · 推理範圍按 Workspace 設定", "Official Hong Kong region · Inference scope follows your workspace");
  copyLabel("#settings-dialog .options-body > small", "圖片生成未接駁：目前官方 Qwen-Image 文件未列香港端點，不會自動轉區。", "Image generation is not connected: the Qwen-Image guide does not list a Hong Kong endpoint. No automatic region switching.");
  ...
  $("#workspace-id").placeholder = t("選填；留空使用香港預設端點", "Optional; blank uses Hong Kong default");
  copyLabel("#about-dialog h2", "分層工作紙生成", "Differentiated worksheet generator");
  copyLabel("#about-dialog .dialog-intro", "可上載 PDF、Word 或圖片，透過香港 Qwen 讀取及改編工作紙，並匯出可編輯的 Word。沒有金鑰也可使用分數示範。內容及難度尚需老師核對，未經官方課程校準。圖片生成、音訊及影片仍未接駁。", "Upload PDF, Word or images, use Hong Kong Qwen to read and adapt them, and export editable Word documents. The fraction demo works without a key. Teachers must review content and difficulty; formal curriculum calibration, generated illustrations, audio and video are not connected.");
