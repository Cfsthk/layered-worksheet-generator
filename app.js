...
function persistPreferences() {
  try {
    localStorage.setItem(PREFS, JSON.stringify({ connectionVersion: 'international-v2', lang: state.lang, presets: state.presets, models: state.models, workspace: $("#workspace-id").value, fallback: $("#fallback").checked, cost: $("#cost-limit").value }));
...
function syncEndpoint() {
  let raw=$("#workspace-id").value.trim() || 'dashscope-intl.aliyuncs.com';
  if (/^[A-Za-z0-9-]{1,80}$/.test(raw)) raw += '.ap-southeast-1.maas.aliyuncs.com';
...
$("#workspace-id").value=storedPreferences.connectionVersion === 'international-v2' ? (storedPreferences.workspace || '') : 'dashscope-intl.aliyuncs.com';
